...
int CSplashWindow::m_millisecondsToDisplay;						// 0 ==> пока нет щелчка мыши или клавиатуры

// create rectangle that product name has to fit in
const int CSplashWindow::m_productNameVerticalOffset = 160;		// промежуток между верхним краем и наименованием продукта
const int CSplashWindow::m_productNameVerticalHeight = 70;		// максимальная высота наименования продукта
const int CSplashWindow::m_productNameLeftMargin = 20;			// промежуток от левого края до имени, компании, копирайта и версии
const int CSplashWindow::m_productNameRightMargin = 20;			// промежуток от правого края до имени, компании, копирайта и версии
const CString CSplashWindow::m_productNameFontName = "Arial";	// шрифт
CSize CSplashWindow::m_productNamePointSize = CSize(-1,-1);		// Размер в пикселях наименования продукта, (-1,-1) ==> вычисление
COLORREF CSplashWindow::m_productNameTextColor = RGB(0,0,0);	// Цвет текста
...
COLORREF CSplashWindow::m_bodyTextColor = RGB(0,255,0);		// color used for company name, copyright and version (-1 ==> use application name color)

// создание прямоугольника сообщения для строки статуса
const int CSplashWindow::m_statusVerticalOffset = 280;			// empty space between top border and top of status string
const int CSplashWindow::m_statusVerticalHeight = 17;			// maximum height of status string
const int CSplashWindow::m_statusLeftMargin = 1;				// distance from left side to place status string
const int CSplashWindow::m_statusRightMargin = 1;				// distance from right side to place status string
...
COLORREF CSplashWindow::m_statusMessageTextColor = RGB(255,255,255);// color used for status message while starting (-1 ==> use application name color)
...
{
	if (!m_bitmap.LoadBitmap(IDB_DRACON))
		return FALSE;
...
{	
	...
	if (verSize != 0)
	{
		...
		LPVOID lplpBuffer;
		BOOL doThis = VerQueryValue (buf, TEXT("\\StringFileInfo\\041904b0\\ProductName"), &lplpBuffer, &ver);
		if ( doThis ) {
			strncpy_s(buf1, (char *)lplpBuffer, ver);
			m_productNameString = (CString)buf1;
		}

		doThis = VerQueryValue (buf, TEXT("\\StringFileInfo\\041904b0\\CompanyName"), &lplpBuffer, &ver);
		if ( doThis ) {
			strncpy_s(buf1, (char *)lplpBuffer, ver);
			m_companyNameString = (CString)buf1;
		}

		doThis = VerQueryValue (buf, TEXT("\\StringFileInfo\\041904b0\\LegalCopyright"), &lplpBuffer, &ver);
		if ( doThis ) {
			strncpy_s(buf1, (char *)lplpBuffer, ver);
			m_copyrightString = (CString)buf1;
		}

		doThis = VerQueryValue (buf, TEXT("\\StringFileInfo\\041904b0\\Comments"), &lplpBuffer, &ver);
		if ( doThis ) {
			strncpy_s(buf1, (char *)lplpBuffer, ver);
			m_commentsString = (CString)buf1;
		}

		doThis = VerQueryValue (buf, TEXT("\\StringFileInfo\\041904b0\\ProductVersion"), &lplpBuffer, &ver);
		if ( doThis ) {
			strncpy_s(buf1, (char *)lplpBuffer, ver);
